...
Pushed once to the client after the server acknowledges `takeOverPane`, so it can paint the current screen before live output arrives. The server does not retain or replay mobile scrollback history.
