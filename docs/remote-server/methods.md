...
- `takeOverPane` acknowledges the request before pushing a [`terminalSnapshot`](events.md) event to the calling client, then streams live [`terminalOutput`](events.md) until released. The server does not retain or replay mobile scrollback history.
